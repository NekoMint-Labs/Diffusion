/**
 * Adapted from React Bits Threads at commit 9481af758aae6cfb34c3652ec40a1c099360331f.
 * MIT + Commons Clause; see docs/third_party/REACT_BITS_MATERIALS_LICENSE.md.
 * Diffusion changes: speed/static control, hard pause while hidden, CSS-pixel resolution cap, shared viewport styling.
 */
...
const Threads: React.FC<ThreadsProps> = ({
  ...
  useEffect(() => {
    ...
    const start = () => {
      if (isVisible && isPageVisible && !animationFrameId.current)
    ...
    function update(t: number) {
      ...
      if (speed > 0 && t - lastRender < 1000 / 30) {
        animationFrameId.current = requestAnimationFrame(update);
        return;
      }
      lastRender = t;
      ...
      if (speed > 0) animationFrameId.current = requestAnimationFrame(update);
    }
    const onVisibility = () => {
  ...
  useEffect(() => { restartRef.current?.(); }, [color, amplitude, distance, speed, enableMouseInteraction]);
