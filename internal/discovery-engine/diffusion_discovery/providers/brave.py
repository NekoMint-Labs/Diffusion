"""Brave as a search source.

One authenticated GET through the shared request client with a bounded tenacity
retry, a timeout path, and classified errors. Auth uses the
``X-Subscription-Token`` header. ``freshness``/``country``/``language`` are sent
ONLY when explicitly given — Brave's language stays unrestricted by default, and
this engine does no language detection.
"""
