...
This is an **in-place redesign of the supplied v0.1 React/TypeScript/Tauri application**. It is not a new static frontend. Paper Day and Graphite Night, restrained surfaces, bilingual copy and a distinct Deep Dive replace the prototype presentation. Core ownership, imperative camera/drag and local-first storage remain.
...
Open the URL printed by Vite (normally `http://127.0.0.1:5173`). No lockfile was supplied and no successful registry resolution was available here. Generate a genuine `package-lock.json` with the install, validate it and commit it; do not use `npm ci` until that exists.
...
The small top-right menu contains only Find, Import/Restore, Export, Settings and Help. Settings changes language and lighting. Help contains the example Field and creation of another empty Field. An explicit example route is available at `/demo?locale=en` or `/demo?locale=zh`; `/demo` uses a separate persistent project ID instead of replacing Main. Existing user project wording is never translated or rearranged when a UI locale changes.
