/**
 * Adapted from React Bits Silk at commit 9481af758aae6cfb34c3652ec40a1c099360331f.
 * MIT + Commons Clause; see docs/third_party/REACT_BITS_MATERIALS_LICENSE.md.
 * Diffusion changes: demand rendering when static/hidden, CSS-pixel DPR, shared viewport styling.
 */
...
const SilkPlane = forwardRef<Mesh, SilkPlaneProps>(function SilkPlane({ uniforms, redrawKey }, ref) {
  ...
  useEffect(() => { invalidate(); }, [redrawKey, invalidate]);
...
const Silk: React.FC<SilkProps> = ({
  ...
  const redrawKey = `${speed}:${scale}:${noiseIntensity}:${color}:${rotation}:${lightMode}`;
  return (
    <div ref={hostRef} className="silk-container">
      <Canvas dpr={1} frameloop={speed > 0 && visible ? 'always' : 'demand'}>
        <SilkPlane ref={meshRef} uniforms={uniforms} redrawKey={redrawKey} />
      </Canvas>
    </div>
  );
};
