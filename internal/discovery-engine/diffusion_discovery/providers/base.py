"""Provider transports and their shared result/error protocol.

Every adapter returns a :class:`ProviderResult` carrying a stable, classified
``error_type`` rather than an upstream message, so a provider's own text can
never reach the envelope. That classification is what lets a failing source and
a working source be told apart instead of collapsing into one failure.
"""
import asyncio
import json
import logging
