...
``Authorization: Bearer`` header. The pure ``normalize_tavily()`` normalizer is
re-exported as ``to_discovery_candidates`` so a captured raw ``results`` list can
be replayed offline, with no network I/O.
"""
